...
int check_stackempty(struct list* list){
	if (list->head == NULL){
                return 1;
        }else{
                return 0;
...
void push(struct list* list, void* val){
...
void* stacktop(struct list* list){
	return list->head->val;


}

void* stackpop(struct list* list){
	struct link* link = list->head;
	void* oldhead = link->val;
	list->head = link->next;
	free(link);
	return oldhead;
