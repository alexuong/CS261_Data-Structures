...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    new_data[physical_index(da->start, i ,da->capacity)] = da->data[physical_index(da->start, i ,da->capacity)];
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  int physicalindex = physical_index(da->start, da->size, da->capacity);
  da->data[physicalindex] = val;
  da->size++;
}
...
void* queuefront(struct dynarray* da){
	return da->data[da->start];


}

void* dequeue(struct dynarray* da){
	void* ptr = dynarray_get(da, da->start);
	da->size--;
	da->start++;
	return ptr;
...
int physical_index(int start, int logical, int capacity){
	int physical = (start + logical) % capacity;
	return physical;
